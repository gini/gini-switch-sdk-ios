...
class AuthenticatorTests: XCTestCase {
    ...
    func testRequestingClientToken() {
        ...
        XCTAssertEqual(tokenResource.url, URL(string: "https://user.gini.net/oauth/token?grant_type=client_credentials"), "The client token request URL doesn't match")
    ...
    func testLoggingInUser() {
        ...
        XCTAssertEqual(userResource.url, URL(string: "https://user.gini.net/oauth/token?grant_type=password"), "The login user request URL doesn't match")
        XCTAssertEqual(userResource.method, "POST", "The create user request method doesn't match")
        XCTAssertEqual(userResource.body, "username=test@gini.net&password=password", "The login user request should contain user credentials in it's payload")
